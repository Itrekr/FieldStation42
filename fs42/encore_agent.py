...
class EncoreAgent:
    ...
    def record_consumption(self, encore_key):
        if not encore_key or encore_key.get("strategy") != "queue":
            return
        cursor_name = encore_key.get("cursor")
        source = encore_key.get("source")
        source_start_time = encore_key.get("source_start_time")
        if not cursor_name or not source or not source_start_time:
            return
        self._pending_cursors[(cursor_name, source)] = datetime.datetime.fromisoformat(source_start_time)
    ...
    @staticmethod
    def reset_airing_history(station_config, cutoff):
        agent = EncoreAgent(station_config)
        with agent._get_connection() as connection:
            cursor = connection.cursor()
            cursor.execute("""
