...
For recurring weekday reruns, prefer a queue cursor instead of a fixed offset when there is a weekend gap. For example, a `prime1_morning` cursor can consume two historical `prime1` broadcasts Monday through Saturday, while a separate `prime1_sunday` cursor consumes six broadcasts on Sundays. The cursors remain independent, so Sunday replay does not move the weekday morning replay position.
